...
local function drawHeldNPC(id, x, y, direction)
	...
	Graphics.drawImageToSceneWP(Graphics.sprites.npc[id].img, x + config.gfxoffsetx - width / 2, y - height + config.gfxoffsety, 0, offsy, width, height, 1, -20)
end
...
function hammerBros.onTickNPC(bro)
	...
	if data.throwTimer >= 0 then
		...
		if not config.quake then
			...
			elseif data.jumpTimer == -1 then
				...
				if bro.collidesBlockBottom and bro.speedY >= 0 then
	...
	elseif data.throwingID ~= nil then
        ...
        local ham2 = NPC.spawn(data.throwingID, bro.x - data.facingDirection * config.throwoffsetx, bro.y - config.throwoffsety, bro:mem(0x146, FIELD_WORD), false)
		ham2.data._basegame.ownerBro = bro
		ham2.direction = data.facingDirection
		ham2.layerName = "Spawned NPCs"
		ham2.speedX = ham2.direction * 1.25 * config.throwspeedx or 0
		ham2.speedY = config.throwspeedy * 0.75 or 0
		ham2.friendly = bro.friendly
        end
