...
--[[CORE]]
littleDialogue.path = 'libs/littleDialogue/'

littleDialogue.eventMessageFix = 0
littleDialogue.eventMessageMaxFrames = 1

littleDialogue.language = nil

function littleDialogue.setPath(path)
	littleDialogue.path = path .. "/"
end

do
	local translation = {}
	
	littleDialogue.translate = function(lang, origText, newText)
		translation[lang] = translation[lang] or {}
		translation[lang][origText] = newText
	end
	
	littleDialogue.setTranslation = littleDialogue.translate
	
	local function findFile(name)
		local resolveFile = Misc.resolveFile(name)
	...
	littleDialogue.loadTranslation = function(name)
		local luaName = (name .. ".lua")
		local fileName = findFile(littleDialogue.path .. "translation/" .. luaName) or findFile("translation/" .. luaName) or findFile(luaName)
		
		translation[name] = loadfile(fileName)()
	end
	
	littleDialogue.loadTranslations = function(name)
		local path = Misc.episodePath() .. littleDialogue.path .. "translation"
		local langs = Misc.listFiles(path)
		
		if #langs == 0 then
			local path = Misc.episodePath() .. "translation"
			langs = Misc.listFiles(path)
		end
		
		for _,name in ipairs(langs) do
			littleDialogue.loadTranslation(name:gsub('.lua', ''))
		end
	end
	
	littleDialogue.getTranslation = function(...)
		local args = {...}
		...
		if translation[lang] and translation[lang][text] then
...
do
    local function setVoice(box,name)
        ...
        if name == nil or name == "" then
            ...
            local path = Misc.resolveSoundFile(littleDialogue.path .. "portraits/".. name) or Misc.resolveSoundFile(littleDialogue.path .. name) or Misc.resolveSoundFile(name)
    ...
    local function loadImage(settings,styleName,name,imageFilename)
        local path = Misc.resolveFile(littleDialogue.path .. styleName.. "/".. imageFilename) or Misc.resolveFile(littleDialogue.path .. imageFilename)
    ...
    local function loadSound(settings,styleName,name,soundFilename)
        settings[name] = Misc.resolveSoundFile(littleDialogue.path .. styleName.. "/".. soundFilename) or Misc.resolveSoundFile(littleDialogue.path .. soundFilename)
    end

    local function findFont(styleName,fontFileName)
        local folderPath = littleDialogue.path .. styleName.. "/".. fontFileName
        if Misc.resolveFile(folderPath) ~= nil then
            return folderPath
        end

        local mainPath = littleDialogue.path .. fontFileName
    ...
    local function compileShader(styleName,vertName,fragName)
        local vertPath = Misc.resolveFile(littleDialogue.path .. styleName.. "/".. vertName) or Misc.resolveFile(littleDialogue.path .. vertName)
        local fragPath = Misc.resolveFile(littleDialogue.path .. styleName.. "/".. fragName) or Misc.resolveFile(littleDialogue.path .. fragName)
    ...
    function littleDialogue.registerStyle(name,settings)
        ...
        settings.typewriterSound = SFX.open(Misc.resolveSoundFile(littleDialogue.path .. name.. "/typewriter") or Misc.resolveSoundFile(littleDialogue.path .. "typewriter"))
    ...
    function customTags.playerKey(fmt,out,args)
        ...
        if currentlyUpdatingBox ~= nil then
            imagePath = imagePath or Misc.resolveGraphicsFile(littleDialogue.path .. currentlyUpdatingBox.styleName.. "/keys/".. imageName.. ".png")
        end

        imagePath = imagePath or Misc.resolveGraphicsFile(littleDialogue.path .. "keys/".. imageName.. ".png")
    ...
    function littleDialogue.getPortraitData(name)
        if portraitData[name] == nil then
            local txtPath = Misc.resolveFile(littleDialogue.path .. "portraits/".. name.. ".txt")
            local data
            ...
            local imagePath = Misc.resolveGraphicsFile(littleDialogue.path .. "portraits/".. name.. ".png")
...
function littleDialogue.create(args)
    ...
    box.text = littleDialogue.getTranslation(args.text or "")
    ...
    box.speakerName = littleDialogue.getTranslation(args.speakerName or "")
...
local function eventMessageRoutine(npcObj, player)
	if runOut then
		if runOut > 0 then
			runOut = runOut - 1
		elseif runOut <= 0 then
	...
	if co then
		local frames = littleDialogue.eventMessageMaxFrames
		
		if frames and frames > 0 then
			Routine.waitFrames(frames)
		end
		
		Routine.run(eventMessageRoutine, npcObj, player)
	else
		return
	end
end
...
function littleDialogue.onMessageBox(eventObj,text,playerObj,npcObj)
	local eventMessageFix = littleDialogue.eventMessageFix
	
	if eventMessageFix ~= 2 then
		littleDialogue.create{
			text = text,
			speakerObj = npcObj or playerObj or player,
		}
		...
		if not eventMessage then
			...
			eventMessage = eventMessage .. "<page>" .. littleDialogue.getTranslation(text)
		end
		
		-- if not co then
		co = Routine.run(eventMessageRoutine, npcObj, player)
		-- end
	end

	if eventMessageFix == 1 then
		Misc.pause()
	end
